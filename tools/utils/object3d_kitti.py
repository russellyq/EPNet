...
#         self.cls_id = cls_type_to_id(self.cls_type)
#         self.truncation = 0
#         self.occlusion = float(label[1])  # 0:fully visible 1:partly occluded 2:largely occluded 3:unknown
#         self.alpha = angle_in_range(np.pi/2 + float(label[-2]))
#         # self.box2d = np.array((float(label[4]), float(label[5]), float(label[6]), float(label[7])), dtype=np.float32)
#         self.box2d = np.array((0,0,50,50), dtype=np.float32)
#         self.h = float(label[4])
#         self.w = float(label[3])
#         self.l = float(label[2])
#         self.loc = np.array(( - float(label[6]), -(float(label[7]) - self.h/2), float(label[5])), dtype=np.float32)
#         self.dis_to_cam = np.linalg.norm(self.loc)
#         self.theta = float(label[8])
#         self.ry = angle_in_range(np.pi/2 - self.alpha*np.pi/180)
#         # (90 + float(label[-1])) / 180 * np.pi
#         self.score = -1.0
#         # self.level_str = 'None'
#         self.level_str = 'Easy'
#         # self.level = self.get_kitti_obj_level()
